...
os.makedirs('Sorted_lists_GALEX_NUV', exist_ok=True)
...
def zero():
    ...
    os.makedirs('Sorted_lists_GALEX_NUV/msrb',exist_ok=True)
    df.round({'RA': 7, 'DEC': 7, 'MSRB': 7, 'Reff': 7, 'MAG_R': 7, 'SERSI': 7}).to_csv('Sorted_lists_GALEX_NUV/msrb/sorted_msrb_catg.csv',index = False)
...
def one():
    ...
    os.makedirs('Sorted_lists_GALEX_NUV/reff',exist_ok=True)
    df[column_order].to_csv('Sorted_lists_GALEX_NUV/reff/sorted_reff_catg.csv',index = False)
...
def two():
    ...
    os.makedirs('Sorted_lists_GALEX_NUV/magtot',exist_ok=True)
    df[column_order].to_csv('Sorted_lists_GALEX_NUV/magtot/sorted_magtot_catg.csv',index = False)
'''
...
def three():
    ...
    os.makedirs('Sorted_lists_GALEX_NUV/seri',exist_ok=True)
    df.round({'RA': 7, 'DEC': 7, 'MSRB': 7, 'Reff': 7, 'MAG_R': 7, 'SERSI': 7}).to_csv('Sorted_lists_GALEX_NUV/seri/sorted_seri_catg.csv',index = False)

loopcycle=False
os.chdir('/home/soumyananda/Dropbox/Goswami_REVISED/DATA_FITS_KIDS_GAMA_VST/GALEX_NUV_FITS_6AS/')
...
while loopcycle==False:
        ...
        if inpval==0:
            ...
            zero()
            loopcycle=True
            os.chdir('/home/soumyananda/Dropbox/Goswami_REVISED/CATALOGRADEC_SPLICED/Sorted_lists_GALEX_NUV/msrb/')
            ...
            #print(val)
            os.makedirs('/home/soumyananda/Dropbox/Goswami_REVISED/DATA_FITS_KIDS_GAMA_VST/GALEX_NUV_FITS_6AS/STACKED_IMAGES_MSRB/',exist_ok=True)
            os.chdir("/home/soumyananda/Dropbox/Goswami_REVISED/DATA_FITS_KIDS_GAMA_VST/GALEX_NUV_FITS_6AS/STACKED_IMAGES_MSRB/")
            f=open("countsinfogalexnuv.txt","w+")
            ...
            for i in range(1,10):
                ...
                for j in range(1, num_lines-2):
                    if  resl <= f2r.iloc[j][4] <= resu:
                        spc=str(f2r.iloc[j][0])+' , '+str(f2r.iloc[j][1])+' , '+str(round(f2r.iloc[j][2],6))+' , '+str(round(f2r.iloc[j][3],6))+' , '+str(f2r.iloc[j][4])+' , '+str(f2r.iloc[j][5])+' , '+str(f2r.iloc[j][6])+' , '+str(f2r.iloc[j][7])
                        mrc.write(spc)
                        mrc.write('\n')
                mrc.close()
                os.chdir('/home/soumyananda/Dropbox/Goswami_REVISED/DATA_FITS_KIDS_GAMA_VST/GALEX_NUV_FITS_6AS/')
                ...
                for k in range(1, n):
                    ...
                    for h in range(0,len(file_list)):
                        ...
                        if np.isclose(btemp1, s1, rtol=1e-4, atol=1e-5, equal_nan=False) and np.isclose(ctemp1, s2, rtol=1e-4, atol=1e-5, equal_nan=False):
                            #print(btemp1-s1)
                            #print(ctemp1-s2)
                            temp = "/home/soumyananda/Dropbox/Goswami_REVISED/DATA_FITS_KIDS_GAMA_VST/GALEX_NUV_FITS_6AS/" + z + "/"
                            shutil.copy2(str(pt), temp)
                image_concat = []
                temp1 = "/home/soumyananda/Dropbox/Goswami_REVISED/DATA_FITS_KIDS_GAMA_VST/GALEX_NUV_FITS_6AS/" + z + "/"
                os.chdir(temp1)
                ...
                for counter in range(0, 10):
                    ...
                    s111 = "stack_galexnuv6as_temp"+ str(counter) + ".fits"
                    os.makedirs(
                        '/home/soumyananda/Dropbox/Goswami_REVISED/DATA_FITS_KIDS_GAMA_VST/GALEX_NUV_FITS_6AS/STACKED_IMAGES_MSRB/',
                        exist_ok=True)
                    os.chdir(
                        "/home/soumyananda/Dropbox/Goswami_REVISED/DATA_FITS_KIDS_GAMA_VST/GALEX_NUV_FITS_6AS/STACKED_IMAGES_MSRB/")
                ...
                s11 = "stack_galexnuv6as_" + str(i) + ".fits"
                s12 = "stack_galexnuv6as_" + str(i) + ".png"
                os.chdir("/home/soumyananda/Dropbox/Goswami_REVISED/DATA_FITS_KIDS_GAMA_VST/GALEX_NUV_FITS_6AS/STACKED_IMAGES_MSRB/")
                shutil.rmtree(temp1)
        ...
        if inpval==0:
            ...
            three()
            loopcycle=True
            os.chdir('/home/soumyananda/Dropbox/Goswami_REVISED/CATALOGRADEC_SPLICED/Sorted_lists_GALEX_NUV/seri/')
            ...
            val = (num_end - num_start) / 12
            #print(val)
            os.makedirs('/home/soumyananda/Dropbox/Goswami_REVISED/DATA_FITS_KIDS_GAMA_VST/GALEX_NUV_FITS_6AS/STACKED_IMAGES_SERI/',exist_ok=True)
            os.chdir("/home/soumyananda/Dropbox/Goswami_REVISED/DATA_FITS_KIDS_GAMA_VST/GALEX_NUV_FITS_6AS/STACKED_IMAGES_SERI/")
            f=open("countsinfogalexnuv1.txt","w+")
            f.write("x,y,yerr,num_files\n")
            os.chdir('/home/soumyananda/Dropbox/Goswami_REVISED/CATALOGRADEC_SPLICED/Sorted_lists_GALEX_NUV/seri/')
            for i in range(1,10):
                resl=num_start + val*(i-1) #incremental dynamic setting for splitting files
                ...
                for counter in range(0, 10):
                    ...
                    s111 = "stack_galexnuv6as_temp"+ str(counter) + ".fits"
                    os.makedirs(
                        '/home/soumyananda/Dropbox/Goswami_REVISED/DATA_FITS_KIDS_GAMA_VST/GALEX_NUV_FITS_6AS/STACKED_IMAGES_SERI/',
                        exist_ok=True)
                    os.chdir(
                        "/home/soumyananda/Dropbox/Goswami_REVISED/DATA_FITS_KIDS_GAMA_VST/GALEX_NUV_FITS_6AS/STACKED_IMAGES_SERI/")
                    hdu1.writeto(s111, overwrite=True)
